...
W_OBJECT_IMPL(VeniceProvider)

VeniceProvider::VeniceProvider(const QString &apiKey, QObject *parent)
    : IAiProvider(parent), m_apiKey(apiKey)
{
}

QStringList VeniceProvider::availableModels() const
{
    return {
        "zai-org-glm-5-1",
        "zai-org-glm-5",
...
void VeniceProvider::generate(const AiRequest &request)
{
    if (m_streaming) {
...
void VeniceProvider::generateStream(const AiRequest &request)
{
    if (m_streaming) {
        emit error("A stream is already in progress");
        return;
...
QString VeniceProvider::buildRequestBody(const AiRequest &request) const
{
    QJsonObject body;
    body["model"] = request.model.isEmpty() ? "zai-org-glm-5-1" : request.model;
...
void VeniceProvider::handleResponse(QNetworkReply *reply)
{
    AiResponse response;
...
void VeniceProvider::handleStreamChunk(const QByteArray &data)
{
